...
- **RegisterTariffUseCase** (`internal/sourcing/application/tariff`): DRAFT契約に料金表を登録
- **ApplyContractToRateUseCase** (`internal/rate/application/rate`): 正式契約化した契約をRateに適用

## ドメインモデル

- **ServiceContract** (`internal/sourcing/domain/contract/service_contract.go`): 契約集約ルート
  - 初期状態: `DRAFT`
